...
from test.validate import local_validate
from task.ahfl_train import execute_epoch
from utils.loss_utils import get_downscale_index
from utils.distill_utils import save_checkpoint
...
class Sever:
    ...
    def fed_train(self, train_set, val_set, user_groups, criterion, args, batch_size, train_params):
        ...
        for round_idx in range(args.start_round, self.num_rounds):
            ...
            print(f'\n | Training on GPU: {args.use_valid} |\n')
    ...
    def fix_idx_array(self, idx_array, local_shape):
        ...
        if all([idx_shape[i] >= local_shape[i] for i in range(len(local_shape))]):
            ...
            idx_shape = self.get_idx_shape(idx_array, local_shape)
        # ind_list被创建为一个长度与idx_array.shape相同的列表，其中每个元素都是slice(None)。slice(None)表示选择该维度的所有元素。
