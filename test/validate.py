...
from utils.distill_utils import accuracy, AverageMeter
from utils.distill_utils import load_state_dict



def local_validate(sever, dataset, user_groups, criterion, args, batch_size, model=None, save=False):
    if model is None:
        load_state_dict(args, sever.global_model)
        model = sever.global_model

    if args.use_valid:
        sever.global_model = sever.global_model.cuda()
        model = model.cuda()

    result_lists = [[] for _ in range(5)]
    local_result_lists = [[[] for _ in range(3)] for _ in range(sever.num_levels + 1)]
    levels = []

    for client_idx in range(args.num_clients):
        client_loader = get_client_dataloader(dataset, user_groups[client_idx], args, batch_size)

        level = sever.get_level(client_idx)
        scale = sever.vertical_scale_ratios[level]
        exit_idx = sever.horizontal_scale_ratios[level]
        local_model = sever.get_local_split(level, scale)
        if args.use_valid:
            local_model = local_model.cuda()

        if level == sever.num_levels - 1:  # local and global models/results are same for highest level
    ...
    results = [sum(result_list) / sever.num_clients for result_list in result_lists]
    local_results = []
    ...
    with open(test_result_filename, 'w') as f:
        for j in range(sever.num_levels):
            text = f'Level {j + 1}/{sever.num_levels} * prec@1 {local_results[j][1]:.3f} prec@5 {local_results[j][2]:.3f}'
            print(text)
            if save:
                print(text, file=f)
        top1 = results[1]
